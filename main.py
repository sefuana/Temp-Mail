...
class MailTmProvider(MailProvider):
    # ✅ Correct API base URL (was broken: contained markdown link syntax)
    BASE_URL = "https://api.mail.tm"
    TIMEOUT_SHORT = aiohttp.ClientTimeout(total=15)
    TIMEOUT_LONG = aiohttp.ClientTimeout(total=20)
    MAX_RETRIES = 3

    async def get_domains(self, session: aiohttp.ClientSession) -> list[str]:
        url = f"{self.BASE_URL}/domains?page=1"
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                logger.info(f"[mail.tm] Fetching domains (attempt {attempt}/{self.MAX_RETRIES})")
                async with session.get(url, timeout=self.TIMEOUT_SHORT) as r:
                    body = await r.text()
                    if r.status == 200:
                        try:
                        ...
                        domains = [d["domain"] for d in members if d.get("isActive", True)]
                        logger.info(f"[mail.tm] Got {len(domains)} domain(s): {domains}")
                        if domains:
                            return domains
                        logger.error(f"[mail.tm] No available domains in response: {body[:300]}")
                    else:
                        logger.error(f"[mail.tm] Domain request failed (HTTP {r.status}): {body[:300]}")
            except asyncio.TimeoutError:
                logger.error(f"[mail.tm] Network timeout on get_domains (attempt {attempt})")
            except aiohttp.ClientConnectionError as e:
                logger.error(f"[mail.tm] Connection error on get_domains: {e}")
            except Exception as e:
                logger.error(f"[mail.tm] Unexpected error on get_domains: {type(e).__name__}: {e}")
            if attempt < self.MAX_RETRIES:
                await asyncio.sleep(2 * attempt)
        logger.error("[mail.tm] All domain fetch attempts failed.")
        return []
    ...
    async def create_account(
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                logger.info(f"[mail.tm] Creating account: {email} (attempt {attempt}/{self.MAX_RETRIES})")
                async with session.post(
                    ...
                    if r.status == 201:
                        try:
                            ...
                            import json as _json
                            data = _json.loads(body)
                        logger.info(f"[mail.tm] Account created successfully: {email}")
                        return data
                    elif r.status == 422:
                        logger.error(f"[mail.tm] Account creation failed (422 - email already exists or invalid): {body[:300]}")
                        # 422 means this email exists or domain invalid — no point retrying same email
                        return None
                    elif r.status == 429:
                        logger.warning(f"[mail.tm] Rate limited (429) on create_account. Waiting...")
                        await asyncio.sleep(5 * attempt)
                        continue
                    else:
                        logger.error(f"[mail.tm] Account creation failed (HTTP {r.status}): {body[:300]}")
            except asyncio.TimeoutError:
                logger.error(f"[mail.tm] Network timeout on create_account (attempt {attempt})")
            except aiohttp.ClientConnectionError as e:
                logger.error(f"[mail.tm] Connection error on create_account: {e}")
            except Exception as e:
                logger.error(f"[mail.tm] Unexpected error on create_account: {type(e).__name__}: {e}")
            if attempt < self.MAX_RETRIES:
                await asyncio.sleep(2 * attempt)
        logger.error(f"[mail.tm] All account creation attempts failed for {email}.")
        return None
    ...
    async def get_token(
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                logger.info(f"[mail.tm] Requesting token for: {email} (attempt {attempt}/{self.MAX_RETRIES})")
                async with session.post(
                    url, json=payload,
                    headers={"Content-Type": "application/json"},
                    timeout=self.TIMEOUT_LONG,
                    ...
                    if r.status == 200:
                        ...
                        if token:
                            logger.info(f"[mail.tm] Token obtained successfully for: {email}")
                            return token
                        logger.error(f"[mail.tm] Token missing from response: {body[:300]}")
                        return None
                    elif r.status == 401:
                        logger.error(f"[mail.tm] Authentication failed (401) for {email}: {body[:300]}")
                        return None
                    elif r.status == 429:
                        logger.warning(f"[mail.tm] Rate limited (429) on get_token. Waiting...")
                        await asyncio.sleep(5 * attempt)
                        continue
                    else:
                        logger.error(f"[mail.tm] Token request failed (HTTP {r.status}): {body[:300]}")
            except asyncio.TimeoutError:
                logger.error(f"[mail.tm] Network timeout on get_token (attempt {attempt})")
            except aiohttp.ClientConnectionError as e:
                logger.error(f"[mail.tm] Connection error on get_token: {e}")
            except Exception as e:
                logger.error(f"[mail.tm] Unexpected error on get_token: {type(e).__name__}: {e}")
            if attempt < self.MAX_RETRIES:
                await asyncio.sleep(2 * attempt)
        logger.error(f"[mail.tm] All token request attempts failed for {email}.")
        return None

    async def get_messages(
        self, session: aiohttp.ClientSession, token: str
    ) -> list[dict]:
        url = f"{self.BASE_URL}/messages?page=1"
        try:
            async with session.get(
                ...
                if r.status == 200:
                    ...
                    logger.warning("[mail.tm] get_messages: token expired or invalid (401)")
                else:
                    body = await r.text()
                    logger.warning(f"[mail.tm] get_messages failed (HTTP {r.status}): {body[:200]}")
        except asyncio.TimeoutError:
            logger.warning("[mail.tm] Network timeout on get_messages")
        except Exception as e:
            logger.warning(f"[mail.tm] get_messages error: {type(e).__name__}: {e}")
        return []

    async def get_message(
        self, session: aiohttp.ClientSession, token: str, msg_id: str
    ) -> Optional[dict]:
        url = f"{self.BASE_URL}/messages/{msg_id}"
        try:
            async with session.get(
                url,
                headers={"Authorization": f"Bearer {token}"},
                ...
                if r.status == 200:
                    ...
                    logger.warning(f"[mail.tm] Message not found: {msg_id}")
                elif r.status == 401:
                    logger.warning("[mail.tm] get_message: token expired or invalid (401)")
                else:
                    body = await r.text()
                    logger.warning(f"[mail.tm] get_message failed (HTTP {r.status}): {body[:200]}")
        except asyncio.TimeoutError:
            logger.warning("[mail.tm] Network timeout on get_message")
        except Exception as e:
            logger.warning(f"[mail.tm] get_message error: {type(e).__name__}: {e}")
        return None
...
T: dict[str, dict[str, str]] = {
    "en": {
        ...
        "generate_fail": "❌ Failed to generate email.\n\nPlease try again later.",
    ...
    "bn": {
        ...
        "generate_fail": "❌ ইমেইল তৈরি করতে ব্যর্থ।\n\nকিছুক্ষণ পর আবার চেষ্টা করুন।",
...
async def create_temp_email(uid: int, session: aiohttp.ClientSession) -> Optional[str]:
    ...
    Tries all available domains in reputation order. Retries with a fresh
    local part on each attempt so address collisions are avoided.
    """
    user = get_user(uid)

    # ── Step 1: Fetch all available domains ───────────────────────────────────
    domains = await ACTIVE_PROVIDER.get_domains(session)
    if not domains:
        logger.error("[Email] No available domains — cannot create email.")
        return None

    # Sort by reputation score, best first
    domains = sorted(domains, key=score_domain, reverse=True)
    logger.info(f"[Email] Will try domains in order: {domains}")

    # ── Step 2: Try each domain until one works ────────────────────────────────
    for domain in domains:
        # Generate a fresh random local part for every attempt
        local = "".join(random.choices(string.ascii_lowercase + string.digits, k=12))
        email = f"{local}@{domain}"
        password = generate_password()

        logger.info(f"[Email] Trying to create: {email}")

        # ── Create account ────────────────────────────────────────────────────
        account = await ACTIVE_PROVIDER.create_account(session, email, password)
        if not account:
            logger.warning(f"[Email] Account creation failed for domain '{domain}', trying next.")
            continue

        # ── Obtain JWT token ──────────────────────────────────────────────────
        # Small delay to let the provider register the account before auth
        await asyncio.sleep(1)
        token = await ACTIVE_PROVIDER.get_token(session, email, password)
        if not token:
            logger.warning(f"[Email] Token request failed for {email}, trying next domain.")
            continue
        ...
        logger.info(
            f"[Email] ✅ Created {email} for uid={uid} "
            f"(domain={domain}, score={score_domain(domain)})"
        )
        return email

    logger.error(f"[Email] ❌ All {len(domains)} domain(s) failed for uid={uid}.")
    return None
