...
# ─── temp-mail.io API ─────────────────────────────────────────────────────────
TEMPMAIL_BASE = "[api.internal.temp-mail.io](https://api.internal.temp-mail.io/api/v3)"
...
T: dict[str, dict[str, str]] = {
    "en": {
        ...
        "get_new_mail_prompt": (
            ...
            "`{email}`\n\n"
            "Would you like to generate a *new* email address?\n"
            "⚠️ _Your current email and all its messages will be permanently discarded._"
        ),
        "generating_new": "⏳ Generating a new temporary email...",
        "new_email_generated": "✅ *New Email Generated!*\n\n`{email}`\n\n_Tap to copy your new email address._",
        "get_new_mail_cancelled": "↩️ Cancelled. Your current email is unchanged.",
    },
    "bn": {
        "welcome": (
        ...
        "get_new_mail_prompt": (
            "📧 *আপনার একটি সক্রিয় ইমেইল আছে:*\n\n"
            "`{email}`\n\n"
            "আপনি কি একটি *নতুন* ইমেইল ঠিকানা তৈরি করতে চান?\n"
            "⚠️ _আপনার বর্তমান ইমেইল এবং সমস্ত বার্তা স্থায়ীভাবে মুছে যাবে।_"
        ),
        "generating_new": "⏳ নতুন অস্থায়ী ইমেইল তৈরি হচ্ছে...",
        "new_email_generated": "✅ *নতুন ইমেইল তৈরি হয়েছে!*\n\n`{email}`\n\n_কপি করতে ট্যাপ করুন।_",
        "get_new_mail_cancelled": "↩️ বাতিল হয়েছে। আপনার বর্তমান ইমেইল অপরিবর্তিত আছে।",
    },
}
...
async def create_temp_email(uid: int, session: aiohttp.ClientSession) -> Optional[str]:
    ...
    Saves email and token to user_data. Wipes old email/seen-messages state.
    """
    user = get_user(uid)
    ...
    seen_messages[uid] = set()  # wipe all seen message history for this user
...
def get_new_mail_keyboard() -> ReplyKeyboardMarkup:
    """Keyboard shown when a user already has an active email."""
    return ReplyKeyboardMarkup(
        [
            [KeyboardButton("Get New Mail 📬")],
            [KeyboardButton("Back 🔙")],
        ],
        resize_keyboard=True,
        is_persistent=True,
...
async def handle_get_mail(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """
    If the user has no active email → generate one immediately.
    If they already have one → show the get_new_mail_keyboard and ask
    whether they want to replace it or go back.
    """
    uid = update.effective_user.id
    user = get_user(uid)

    if not user.get("email"):
        # ── No existing email: generate one straight away ──────────────────
        ...
        # Switch the reply keyboard to main_keyboard after generation
        await update.message.reply_text(
            "✅",
            reply_markup=main_keyboard(uid),
        )
    else:
        # ── Existing email: ask whether to replace or go back ───────────────
        await update.message.reply_text(
            t(uid, "get_new_mail_prompt", email=user["email"]),
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=get_new_mail_keyboard(),
        )


async def handle_get_new_mail(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """
    User confirmed they want a brand-new email.
    Wipe the old email + seen_messages, generate fresh, show main_keyboard.
    """
    uid = update.effective_user.id
    user = get_user(uid)

    # Wipe old state explicitly before generating
    user["email"] = None
    user["token"] = None
    ...
    if not email:
        await msg.edit_text(t(uid, "generate_fail"))
        # Restore main keyboard even on failure
        await update.message.reply_text(
            t(uid, "back_to_menu"),
            reply_markup=main_keyboard(uid),
        )
    ...
    await msg.edit_text(
        t(uid, "new_email_generated", email=email),
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=get_mail_inline(uid),
    )
    ...
    logger.info(f"[Email] User uid={uid} replaced their email → {email}")


async def handle_back_from_new_mail(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """
    User chose 'Back 🔙' from the get_new_mail_keyboard.
    Cancel and return to main_keyboard without touching the current email.
    """
    uid = update.effective_user.id
    await update.message.reply_text(
        t(uid, "get_new_mail_cancelled"),
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=main_keyboard(uid),
    )
...
def main() -> None:
    ...
    # ── New-mail confirmation flow (must be registered BEFORE the generic
    #    "🔙 Back" handler so the more-specific regex wins first) ────────────
    app.add_handler(MessageHandler(
        filters.Regex(r"^Get New Mail 📬$"), handle_get_new_mail
    ))
    app.add_handler(MessageHandler(
        filters.Regex(r"^Back 🔙$"), handle_back_from_new_mail
    ))
